...
class App:
    def __init__(self):
        ...
        self.input_layout = [[sg.Button('Video')],
                            ...
                            sg.Column([ [sg.Button("Remove", key="-remove-roi")],
    ...
    def draw_results(self):
        ...
        for i, r in enumerate(self.current_results):
            ...
            if i == selected_index:
                color = "red"
            self.figures.append(self.graph.draw_line((x, y1), (x, y2), color=color))
            self.figures.append(self.graph.draw_line((x1, y), (x2, y), color=color))
            self.figures.append(self.graph.draw_text(f"{int(r[5])} {r[4] * 100.0:.1f}", (x1, y1), color=color))
    ...
    def refresh_roi_list(self):
        ...
        self.window.Element('-roi-list').Widget.selection_set(selected_index)
    ...
    def mainloop(self):
        ...
        while self.running:
            ...
            if event == '-up':
                ...
                if len(i)>0:
                    i=i[0]
                    self.current_results[i][1]-=1
                    self.current_results[i][3] -= 1
                    self.refresh_roi_list()
                    self.draw_results()
            ...
            if event == '-left':
                ...
                if len(i)>0:
                    i=i[0]
                    self.current_results[i][0] -= 1
                    self.current_results[i][2] -= 1
                    self.refresh_roi_list()
                    self.draw_results()
